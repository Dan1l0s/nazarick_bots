...
class logger:
    ...
    def skip(self, ctx):
        f = open(f'{abs_path}.txt', "a", encoding = 'utf-8')
        f.write(
            datetime.datetime.now().strftime("%H:%M:%S") + f" : SKIP : Skipped track at {ctx.guild.name} / {ctx.guild.voice_client.channel}\n")
        f.close()
    ...
    def playing(self, ctx):
        f = open(f'{abs_path}.txt', "a", encoding = 'utf-8')
        f.write(
           datetime.datetime.now().strftime("%H:%M:%S") + f" : PLAY : Playing {self.songs_queue[ctx.guild.id][0]['title']} in vc: {ctx.guild.name} / {ctx.guild.voice_client.channel}\n")
        f.close()
